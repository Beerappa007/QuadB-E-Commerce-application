...
function AllProducts() {
  ...
  const handleCategory = () => {
    console.log(category, color, price);
    axios
      .get(
        `http://localhost:4000/api/products/getProductsByCategory?category=${category}&color=${color}&price=${price}`
      )
      .then((res) => {
        console.log(res);
